...
import io.github.minthem.csvparser.annotation.CsvField
import io.github.minthem.csvparser.config.CsvConfig
import io.github.minthem.csvparser.config.WriterConfig
import io.github.minthem.csvparser.converter.CsvConverter
import io.github.minthem.csvparser.exception.CsvEntityMappingException
import io.github.minthem.csvparser.exception.CsvFieldConvertException
 ...
 *
 * Errors:
 * - [CsvEntityMappingException] when indexes are duplicated/invalid
 *   or no columns are found.
 * - [CsvFieldConvertException] when a value cannot be converted.
 * - [io.github.minthem.csvparser.exception.CsvUnsupportedTypeException] for unsupported property types.
 *
...
class CsvEntityWriter<T : Any>(
    ...
    private val valueProperties: List<CsvConverterAdapter<*>?> by lazy {
        buildHeaderIndexMap()
    }
     ...
     *
     * @throws CsvFieldConvertException when conversion fails
     * @throws io.github.minthem.csvparser.exception.CsvUnsupportedTypeException when the property type is unsupported
     */
    ...
    private fun buildHeaderIndexMap(): List<CsvConverterAdapter<*>?> {
        val ctor = entityClass.primaryConstructor!!
        val params = ctor.parameters
        val props = entityClass.memberProperties.associateBy { it.name }

        val headerIndexMap = mutableMapOf<Int, CsvConverterAdapter<*>>()
        val unIndexProperties = mutableListOf<CsvConverterAdapter<*>>()
        ...
        for (param in params) {
            ...
            val cell = createAdapter(headerName, prop)
    ...
    private fun <U> createAdapter(
        headerName: String,
        prop: KProperty1<*, U>,
    ): CsvConverterAdapter<U> {
...
private class CsvConverterAdapter<T>(
    ...
    private val converter: CsvConverter<T>,
) {
    fun serialize(value: Any?): Result<String?> {
        @Suppress("UNCHECKED_CAST")
        return converter.serialize(value as T?)
    }
}
